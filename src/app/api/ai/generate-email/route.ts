...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey || apiKey === 'placeholder') {
      ...
      const templateContent = generateTemplateEmail(prompt, subject, campaignName)
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert email marketing copywriter. Create professional, engaging, and conversion-focused email content. Use HTML formatting for structure. Include a clear call-to-action.'
          },
          {
            role: 'user',
...
Please provide the email body in HTML format with proper structure, compelling copy, and a clear call-to-action button.`
    ...
    const { prompt, subject, campaignName } = await req.json()
    const templateContent = generateTemplateEmail(prompt, subject, campaignName)
...
function generateTemplateEmail(prompt: string, subject: string, campaignName: string): string {
...
function extractKeyMessage(prompt: string): string {
  ...
  if (sentences.length > 0) {
    return sentences[0].trim() + '. We have something special for you that you won\'t want to miss.'
  }
...
function extractAdditionalDetails(prompt: string): string {
  ...
  if (prompt.toLowerCase().includes('gift') || prompt.toLowerCase().includes('bonus')) {
    return 'Order now and receive an exclusive bonus gift with your purchase!'
  }
  if (prompt.toLowerCase().includes('member') || prompt.toLowerCase().includes('exclusive')) {
...
function extractPSMessage(prompt: string): string {
  if (prompt.toLowerCase().includes('hurry') || prompt.toLowerCase().includes('limited')) {
