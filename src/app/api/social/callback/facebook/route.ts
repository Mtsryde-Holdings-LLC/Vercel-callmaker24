...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/dashboard/social?error=${encodeURIComponent(error === 'access_denied' ? 'user_cancelled' : 'oauth_error')}`)
  }
  ...
  try {
    ...
    if (!tokenData.access_token) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/dashboard/social?error=no_access_token`)
    }
    ...
    if (!user) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/dashboard/social?error=user_not_found`)
    }
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/dashboard/social?connected=Facebook&username=${encodeURIComponent(userData.name)}`)
  } catch (error) {
