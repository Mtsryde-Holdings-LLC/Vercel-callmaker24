...
export const POST = withApiHandler(
  async (request: NextRequest, { session, requestId }: ApiContext) => {
    const { contentType, product, tone, targetAudience, customPrompt } =
      await request.json();
    ...
    const caption =
      captions[tone as keyof typeof captions] || captions.professional;
  ...
  { route: "POST /api/social/generate-content" },
);
