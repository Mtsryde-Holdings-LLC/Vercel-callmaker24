...
export async function GET(
  ...
  try {
    ...
    console.log(`Connected ${platform} account:`, accountData);

    // In production: await prisma.socialAccount.create({ data: accountData });

    // Redirect back to social media page with success
    return NextResponse.redirect(
      new URL(`/dashboard/social?connected=${platform}&username=${encodeURIComponent(accountData.username)}`, request.url)
    );
  } catch (error) {
    console.error('OAuth callback error:', error);
    return NextResponse.redirect(
      new URL('/dashboard/social?error=callback_failed', request.url)
    );
  }
}
...
async function getUserProfile(platform: string, accessToken: string): Promise<any> {
  try {
    switch (platform) {
      ...
      default:
        return { id: Date.now(), username: `${platform}_user`, name: `${platform} User` };
    }
  } catch (error) {
