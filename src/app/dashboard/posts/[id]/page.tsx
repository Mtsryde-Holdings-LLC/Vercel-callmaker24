...
export default function PostDetailPage() {
  ...
  const markAsPosted = async () => {
    if (!confirm('Mark this post as posted? This will set the posted date to now.')) {
      return;
    }
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button
                      onClick={() => copyToClipboard(latestVersion.caption, 'caption')}
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-1 text-sm bg-blue-50 text-blue-600 rounded hover:bg-blue-100"
                      >
                        {copiedHashtags ? <Check size={14} /> : <Copy size={14} />}
                ...
                <div className="text-xs text-gray-500">
                  {latestVersion.source === 'AI_GENERATED' ? '🤖 AI Generated' : '✏️ User Edited'} • 
                  Created by {latestVersion.createdBy.name || latestVersion.createdBy.email}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-2">
              ...
              {post.status === 'POSTED' && (
                <button
                  onClick={() => router.push(`/dashboard/posts/${postId}/analytics`)}
          ...
          {post.performance.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Latest Performance</h3>
              <div className="space-y-3">
                {post.performance[0].likes !== null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Likes</span>
                    <span className="font-medium">{post.performance[0].likes}</span>
                  </div>
                )}
                {post.performance[0].comments !== null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Comments</span>
                    <span className="font-medium">{post.performance[0].comments}</span>
                  </div>
                )}
                {post.performance[0].shares !== null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Shares</span>
                    <span className="font-medium">{post.performance[0].shares}</span>
                  </div>
                )}
                {post.performance[0].reach !== null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Reach</span>
                    <span className="font-medium">{post.performance[0].reach}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">
              {post.platform === 'INSTAGRAM' && 'Best times: 11am-1pm, 7pm-9pm. Use 20-30 hashtags for maximum reach.'}
              {post.platform === 'FACEBOOK' && 'Best times: 1pm-4pm. Focus on engaging questions and visual content.'}
              {post.platform === 'TWITTER_X' && 'Best times: 9am-3pm. Keep it concise, use 1-2 hashtags.'}
              {post.platform === 'LINKEDIN' && 'Best times: Tue-Thu, 10am-12pm. Professional tone, industry insights.'}
              {post.platform === 'TIKTOK' && 'Best times: 6pm-10pm. Trending sounds, quick hooks, vertical video.'}
            </p>
          </div>
        </div>
      </div>
