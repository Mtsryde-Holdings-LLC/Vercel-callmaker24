    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          scheduledFor: campaign.scheduledAt ? new Date(campaign.scheduledAt).toISOString().slice(0, 16) : '',
        })
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit SMS Campaign</h1>
          <p className="text-gray-600 mt-1">Update your SMS campaign</p>
        </div>
        <Link href={`/dashboard/sms/${params.id}`} className="text-gray-600 hover:text-gray-900">
          ← Back
        </Link>
      </div>
