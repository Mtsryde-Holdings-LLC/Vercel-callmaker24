...
function ShopifyCheckoutForm({
  ...
  const handleShopifyCheckout = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Failed to create Shopify billing charge",
        );
      }
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-700 mt-1">
              As a Shopify merchant, your subscription is managed through
              Shopify&apos;s billing system. You&apos;ll be redirected to
              Shopify to approve the charge. The subscription will appear on
...
function ActiveSubscriptionBanner({
  ...
  const planConfig =
    SUBSCRIPTION_PLANS[billingInfo.currentPlan as SubscriptionTier];
        ...
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-semibold">
              Current Plan: {planConfig?.name || billingInfo.currentPlan}
            </h3>
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                billingInfo.currentStatus === "ACTIVE"
                  ? "bg-green-100 text-green-800"
                  : billingInfo.currentStatus === "TRIALING"
...
export default function SubscriptionPage() {
  ...
  const [billingInfo, setBillingInfo] = useState<BillingProviderInfo | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (shopifyBilling === "success" && plan) {
      setSuccessMessage(
        `Successfully subscribed to the ${SUBSCRIPTION_PLANS[plan as SubscriptionTier]?.name || plan} plan via Shopify!`,
      );
      fetchBillingProvider();
    }
    if (error) {
  ...
  const plans = Object.entries(SUBSCRIPTION_PLANS) as [
    SubscriptionTier,
    (typeof SUBSCRIPTION_PLANS)[SubscriptionTier],
  ][];
  const currentPlan = SUBSCRIPTION_PLANS[selectedPlan];
  const priceId =
    process.env[
      `NEXT_PUBLIC_STRIPE_PRICE_ID_${selectedPlan}_${billingPeriod.toUpperCase()}`
    ] ||
        ...
        {billingInfo?.hasActiveSubscription &&
          billingInfo.currentPlan !== "FREE" && (
            <ActiveSubscriptionBanner
              ...
              onManage={() => {
                if (
                  billingInfo.billingProvider === "shopify" &&
                  billingInfo.shopifyChargeId
                ) {
                  // Shopify merchants manage billing in Shopify admin
                  window.open(
                    `https://${billingInfo.shopifyShop}/admin/settings/billing`,
                    "_blank",
                  );
                } else {
          ...
          <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-700 mt-1">
                Your Shopify store is connected. All subscription charges will
                be processed through Shopify&apos;s billing system and will
                appear on your Shopify invoice. This is required by the Shopify
                App Store.
              </p>
            </div>
          </div>
                ...
                <div className="text-center">
                  ...
                  <ul className="space-y-2 mb-6 text-left text-xs">
                    ...
                    <li className="flex items-center gap-2">
                      ...
                      {plan.features.maxCustomers === -1
                        ? "Unlimited"
                        : plan.features.maxCustomers.toLocaleString()}{" "}
                      customers
                    </li>
                    <li className="flex items-center gap-2">
                      <svg
                      ...
                      {plan.features.maxEmailsPerMonth === -1
                        ? "Unlimited"
                        : plan.features.maxEmailsPerMonth.toLocaleString()}{" "}
                      emails/mo
                    </li>
                    <li className="flex items-center gap-2">
                      <svg
                      ...
                      {plan.features.maxSMSPerMonth === -1
                        ? "Unlimited"
                        : plan.features.maxSMSPerMonth.toLocaleString()}{" "}
                      SMS/mo
                    </li>
                    <li className="flex items-center gap-2">
                      <svg
                      ...
                      {plan.features.maxVoiceMinutesPerMonth === -1
                        ? "Unlimited"
                        : plan.features.maxVoiceMinutesPerMonth.toLocaleString()}{" "}
                      voice min/mo
                    </li>
                    <li className="flex items-center gap-2">
                      <svg
                      ...
                      {plan.features.maxSocialAccounts === -1
                        ? "Unlimited"
                        : plan.features.maxSocialAccounts}{" "}
                      social accounts
                    </li>
