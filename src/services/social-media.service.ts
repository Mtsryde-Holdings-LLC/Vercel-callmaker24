...
export class SocialMediaService {
  ...
  static async publishPost(postId: string) {
    ...
    try {
      ...
      switch (post.platform) {
        case "FACEBOOK":
          ({ platformPostId, platformUrl } = (await this.publishToFacebook(
            post,
          )) as any);
          break;
        case "INSTAGRAM":
          ({ platformPostId, platformUrl } = (await this.publishToInstagram(
            post,
          )) as any);
          break;
        case "TWITTER":
          ({ platformPostId, platformUrl } = (await this.publishToTwitter(
            post,
          )) as any);
          break;
        case "LINKEDIN":
          ({ platformPostId, platformUrl } = (await this.publishToLinkedIn(
            post,
          )) as any);
          break;
        case "TIKTOK":
          ({ platformPostId, platformUrl } = (await this.publishToTikTok(
            post,
          )) as any);
          break;
        default:
      ...
      "https://api.linkedin.com/v2/ugcPosts",
      {
        ...
        specificContent: {
          "com.linkedin.ugc.ShareContent": {
            ...
            shareMediaCategory:
              mediaUrls && mediaUrls.length > 0 ? "IMAGE" : "NONE",
          },
        },
        visibility: {
  ...
  private static async getFacebookPostEngagement(
    postId: string,
    accessToken: string,
  ) {
    const response = await axios.get(
  ...
  private static async getInstagramPostEngagement(
    postId: string,
    accessToken: string,
  ) {
    const response = await axios.get(
  ...
  private static async getTwitterPostEngagement(
    postId: string,
    accessToken: string,
  ) {
    const response = await axios.get(
  ...
  private static async getLinkedInPostEngagement(
    postId: string,
    accessToken: string,
  ) {
    const response = await axios.get(
